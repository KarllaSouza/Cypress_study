...
describe('System tests - Form fields', () => {
    ...
    context('fill out the form', () => {
        ...
        it('Test case 2 - Select and deselect each of the "FAVORITE DRINK" checkboxes', () => {
            cy.get('#drink1').should('be.visible').check().wait(300)
            cy.get('[data-testid="drink1"]').should('be.checked')
            cy.get('#drink2').should('be.visible').check().wait(300)
            cy.get('[data-testid="drink2"]').should('be.checked')
            cy.get('#drink3').should('be.visible').check().wait(300)
            cy.get('[data-testid="drink3"]').should('be.checked')
            cy.get('#drink4').should('be.visible').check().wait(300)
            cy.get('[data-testid="drink4"]').should('be.checked')
            cy.get('#drink5').should('be.visible').check().wait(300)
            cy.get('[data-testid="drink5"]').should('be.checked')
            cy.get('#drink1').should('be.visible').uncheck().wait(300)
            cy.get('[data-testid="drink1"]').should('not.be.checked')
            cy.get('#drink2').should('be.visible').uncheck().wait(300)
            cy.get('[data-testid="drink2"]').should('not.be.checked')
            cy.get('#drink3').should('be.visible').uncheck().wait(300)
            cy.get('[data-testid="drink3"]').should('not.be.checked')
            cy.get('#drink4').should('be.visible').uncheck().wait(300)
            cy.get('[data-testid="drink4"]').should('not.be.checked')
            cy.get('#drink5').should('be.visible').uncheck().wait(300)
        ...
        it('Test case 4: Select options from a dropdown', () => {
            cy.get('#automation').should('be.visible').wait(200)
            cy.get('#automation').select('yes').should('have.value', 'yes').wait(200)
            cy.get('#automation').select('no').should('have.value', 'no').wait(200)
            cy.get('#automation').select('undecided').should('have.value', 'undecided').wait(200)
        });
        ...
        it('Test case 7 - Fill in the message field', () => {
            ...
            cy.get('#message').type(textoCurto, {delay: 110})
                .should('have.value', textoCurto)
                .clear()
                .should('have.value', '')

            //Teste 7 - Extra 1 - Prencher campo message com texto longo
            cy.get('#message').type(citacaoLonga, {delay: 110})
        ...
        it('Test case 8 - Fill in the message field and clear it later', () => {
            cy.get('#message')
                .type('Teste realizado com sucesso.', {delay: 50})
    ...
    context('fill out  and send the form - complete flow ', () => {
        it('Test case 9 - FILL OUT THE FORM COMPLETELY WITH ASSERTIONS AND SEND', () => {
            cy.get('#name-input').type('Karlla', {delay: 150})
            cy.get('#drink1').should('be.visible').check().wait(200)
            cy.get('[data-testid="drink1"]').should('be.checked')
            cy.get('#drink5').should('be.visible').check().wait(200)
            cy.get('[data-testid="drink5"]').should('be.checked')
            cy.get('#color3').should('be.visible').check().wait(200)
            cy.get('[data-testid="color3"]').should('be.checked')
            cy.get('#automation').should('be.visible').wait(200)
            cy.get('#automation').select('no').should('have.value', 'no').wait(500)
            cy.get('#email').type('karlla@email.com', {delay: 110})
